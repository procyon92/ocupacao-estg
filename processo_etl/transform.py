...
class DataTransformer:
    ...
    def _get_semester_start(self, year: int, semester: int) -> pd.Timestamp:
        ...
        if semester == 1:
            ...
            days_until_monday = (7 - sep_1.dayofweek) % 7
            first_monday = sep_1 + pd.DateOffset(days=days_until_monday)
            return first_monday + pd.DateOffset(weeks=2)
        else:
    ...
    def build_date_dimension(self, start_date='2018-01-01', end_date='2035-12-31') -> pd.DataFrame:
        ...
        def _classify_semestre(row):
            # Classifica pelo semestre com base nas datas reais — não apenas pelo mês
            ...
            # Para jan-jul, o Sem 1 de referência é o do ano anterior
            ...
            return 0  # entre semestres / férias

        df['Semestre'] = df.apply(_classify_semestre, axis=1)

        # Setembro marca a transição do ano letivo
        ...
        def _escolar_week(row):
            # Semana letiva contada a partir do início do semestre (começa em 1)
            if row['Semestre'] == 0:
                return 0
            ...
            ano_ref  = ano if (row['Semestre'] == 1 and mes >= 9) else ano
    ...
    def _apply_business_filters(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if col_i and col_f:
            ...
            dropped = pre - len(df)
            if dropped > 0:
                self.logger.info(f"[OUTLIERS] {dropped:,} registos com duração > 6h ou <= 0 removidos.")

            esp_c = next((c for c in ['espaco', 'nome_espaco'] if c in df.columns), None)
            if esp_c:
                # Marca duplicados (mesmo espaço + hora) como eventos agregados
    ...
    def _classify_epoca(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if col_i:
            ...
            conditions = [mes.isin([1, 2]), mes.isin([6, 7]), mes == 8]
    ...
    def _align_to_schema(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if uc_code_col:
            df[uc_code_col] = df[uc_code_col].astype(str).str.replace(r'\.0$', '', regex=True).str.strip()
        ...
        if 'nome_curso'    in df.columns: rename_map['nome_curso']    = 'Nome_Curso'
        if 'codigo_curso'  in df.columns: rename_map['codigo_curso']  = 'Codigo_Curso'
