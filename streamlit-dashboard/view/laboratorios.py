...
class LaboratoriosProfile(BaseProfile):
    def render(self, filters: Filters) -> None:
        ...
        # Seletor de departamento específico desta view — independente do filtro global
        dept_map      = get_departamentos()
        dept_labels   = list(dept_map.keys())
        DEFAULT_LABEL = "Engenharia Informática"
        label_opts    = [Omisso.ALL_DEPTS] + dept_labels
        default_idx   = label_opts.index(DEFAULT_LABEL) if DEFAULT_LABEL in label_opts else 0

        selected_label = st.selectbox(
            "Departamento", options=label_opts, index=default_idx, key="lab_dept_select"
        )

        # Copia os filtros globais e força categoria = Laboratório
        ...
        if selected_label != Omisso.ALL_DEPTS:
        ...
        with k5: render_kpi("Sessões Vazias",      f"{ghost_count:,}", "👻", GHOST)
